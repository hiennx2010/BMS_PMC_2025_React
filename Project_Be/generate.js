...
process.argv.forEach((arg) => {
  if (arg.startsWith('--src')) {
    ...
    arg.startsWith('--junction-model') ||
    arg.startsWith('associative-entity')
  ) {
...
function genEntity(name) {
  ...
  let __filePath = `${__src}/entity/${_.kebabCase(name)}/${_.kebabCase(
    name,
  )}.entity.ts`;
...
function genController(name) {
  ...
  let __filePath = `${__src}/controller/${_.kebabCase(name)}/${_.kebabCase(
    name,
  )}.controller.ts`;
  let rs = fs.existsSync(__filePath);
  if (rs) {
  ...
  let __content = fs
    .readFileSync('./src/common-module/.template/controller.temp')
    ?.toString();
...
function genService(name) {
  ...
  let __filePath = `${__src}/service/${_.kebabCase(name)}/${_.kebabCase(
    name,
  )}.service.ts`;
  let rs = fs.existsSync(__filePath);
  if (rs) {
  ...
  let __content = fs
    .readFileSync('./src/common-module/.template/service.temp')
    ?.toString();
  __content = __content.replaceAll(/@@entity@@/g, __name);
  __content = __content.replaceAll(/@@entityKebabCase@@/g, _.kebabCase(name));
